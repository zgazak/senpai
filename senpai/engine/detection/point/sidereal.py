...
def extract_point_sources(
    ...
    # First pass: detect sources for FWHM estimation. Sample deep (300): the
    # brightest sources are almost all SATURATED (and a bright saturated star
    # spawns many adjacent detections along its bloom), so we must reach past them
    # to the fainter unsaturated stars. The reject + dedup below do the filtering.
    ...
    # Measure FWHM only from unsaturated, de-blended stars, via a fit-free
    # half-max-area measure (the Gaussian fit returns garbage on these messy /
    # saturated PSFs). Dedup so a saturated star's bloom-duplicates don't crowd
    # out the unsaturated sample.
    sat_level = _estimate_saturation_level(image.data, detected_sources)
    fwhms: list[float] = []
    ...
    for source in detected_sources:
        ...
        if any((xc - mx) ** 2 + (yc - my) ** 2 < DEDUP_R2 for mx, my in measured_pos):
            continue
        try:
            fwhm = _robust_source_fwhm(image.data, xc, yc, sat_level)
        except Exception as e:
